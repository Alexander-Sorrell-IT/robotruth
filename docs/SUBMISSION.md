...
One engine — ~590 lines, no ML libraries — is the single source of truth behind the web API, the MCP server, the per-repo scorecard, and the CLI (a pip-installable console script: `robotruth <pr-url>`). **No adapter contains audit logic.** The category lives in the function, not the surface.
...
- **Deploy Receipt** — [`/deploy/example`](https://robotruth-rdft.vercel.app/deploy/example) is a real, deterministic second surface: a deployment manifest diff audited by the **same pure grader** as the PR receipt (`grade_and_verdict` — no model in the verdict path), computed live, not authored. The scanners differ (`manifest_drift`, `env_var_surface`, `probe_changes`, `rollout_target`); the grader and grammar are identical. The Protocol is *demonstrated* across two surfaces, not asserted — one engine, two surfaces, same verdict words at `resource:line`.
...
- **Verification:** 96 pytest tests — 81 engine (claim extraction, all four PR scanners, the deploy-surface scanners, grading logic) plus 13 API-endpoint tests and 2 MCP-server tests. TypeScript strict mode passes on the web surface. Every scanner has regression tests pinning the precision decisions made during the build.
