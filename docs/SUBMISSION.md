...
- **Verification:** 88 pytest tests — 73 engine (claim extraction, all four scanners, grading logic) plus 13 API-endpoint tests and 2 MCP-server tests. TypeScript strict mode passes on the web surface. Every scanner has regression tests pinning the precision decisions made during the build.
